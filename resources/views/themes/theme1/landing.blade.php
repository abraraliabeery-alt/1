...
<div id="fab" aria-label="اختصار الاتصال">
  ...
  <main>
    ...
    <div id="properties" aria-hidden="true"></div>
    ...
    <section id="partners" class="tpl-clients section reveal">
      <div class="container center">
        <h2 class="title-deco">الشركاء</h2>
        <div class="clients-row" style="margin-top:14px">
          @forelse(($partners ?? collect()) as $p)
            <div class="client">
              @if($p->logo_url)
                <a href="{{ $p->website_url ?: '#' }}" target="_blank" rel="noopener">
                  <img src="{{ $p->logo_url }}" alt="{{ $p->name }}" onerror="this.style.opacity=.35">
                </a>
              @else
    ...
    <div id="testimonials" aria-hidden="true"></div>
    ...
    <div id="faqs" aria-hidden="true"></div>
